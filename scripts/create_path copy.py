...
class AR_path_Node():
    ...
    def ar_pose_callback(self,data):
            ...
            elif len(self.ar_maker_data) ==0 and self.detect_target_ar_flg != True or self.detect_target_ar_flg != True and self.ar_id != self.target_id:
                print("maigo")
                self.detect_target_ar_flg =False
                self.search_ar()
    # def ar_pose_callback(self,data):
    ...
    def track_ar(self):
        ...
        if self.move_flg:
            if (self.track_flg):
                ...
                if (abs(self.target_pose_z) >= self.liner_thr or abs(self.target_pose_x) >= self.ang_thr):
                    self.vel.linear.x = max(min(0.05 * (self.target_pose_z - 0),0.2),-0.2)
